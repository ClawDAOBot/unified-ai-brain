...
/**
 * Project lifecycle stage — moved inline from poa-cli/src/lib/brain-projections.ts
 * (Stage 2 of task #463 extraction). Small standalone type; if the projections
 * layer also moves to core later, consolidate then.
 */
type ProjectStage =
    ...
    default:
      warnings.push(
        `unknown doc id "${docId}" — no schema registered, accepting any shape. ` +
          `Add a validator to packages/core/src/schemas.ts to enforce one.`,
      );
  }
